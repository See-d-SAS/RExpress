...
exports.prevent_free_routes = false
exports.prevent_free_scripts = true
...
if(!exports.prevent_free_routes){
	app.post('/R/:function', function (request, response) {
		request.Rfunction = null
...
if (!exports.prevent_free_scripts) {
...
// call the function defined in the main.js
const handleCustomRoutes = () => {
	app.post('/R/*', function(request, response){
		if(!request.Rfunction){
			response.status(404).send('Rfunction is null')
		}else {
			let call = request.Rfunction + '('
			let args = []
			if(request.body){
				for(let key in request.body){
					let value = request.body[key]
